...
$title = 'Catalogo de Revisores'; // Establece el título de la página
...
echo '<form method="post" 
            ...
            action="AP_003_001n.php?ID='.$_GET['ID'].'&A='.$_GET['A'].'" />';
...
if ($_GET['A']=='vDet' And $_POST['BtnSav_Det']=='') {
    $Sql = "SELECT * FROM Cat_temas WHERE STATUS = 'A'";
    $Res = DB_query($Sql,$db);
    // Guardar los temas en un array
    $temasDisponibles = [];
    while ($Row = DB_fetch_array($Res)) {
        $temasDisponibles[] = $Row;
    }
}

// si A toma el valor de vMod entonces inserta estas variables en el formulario
if ($_GET['A']=='vMod' And $_POST['BtnSav']=='') {
    $Sql_Ase = "SELECT * FROM Cat_Rev_main WHERE ID = '".$SelID."'";
...
// al dar click en guardar insertar los datos de los campos en la database detalle
if ($_POST['BtnSav_Det']=='Guardar') {
    foreach ($_POST['temas'] as $tema_id){
        $sql = "INSERT INTO Cat_Rev_det  (ID_rev, tema) 
                                    VALUES ( '".$SelID."'
                                            ,'".$tema_id."')";
        $res  = DB_query($sql,$db);
        ...
        $Msg  = 'Se asignaron temas a Revisor';
        $MsgC = 'green'; 
        unset ($_POST['temas']);
        $vSaleM = 'OUT';
}
...
if ($_POST['BtnSav']=='Guardar') {
    $sql = "INSERT INTO Cat_Rev_main  (nombre, mail, status) 
                                    VALUES ( '".$_POST['nombre']."'
                                            ,'".$_POST['mail']."'
                                            ,'A')";
        $res  = DB_query($sql,$db);
        $Msg  = 'Se agrego Nuevo Revisor '.$_POST['nombre'];
        $MsgC = 'green'; 
        unset ($_POST['nombre']);
        unset ($_POST['mail']);
        $vSaleM = 'OUT';
...
if ($_GET['A']=='vEli') {
    // Construye una consulta SQL para eliminar el ID seleccionado.
        $Del_CC = "UPDATE Cat_Rev_main
                    SET status = 'D'
                    WHERE ID =".$SelID;
        $Res_CC = DB_query($Del_CC,$db);  
        prnMsgV20('Se ha eliminado el Revisor : '.$SelID.' ...','success');
}
...
if ($_POST['BtnSav'] == 'Modificar') {
    ...
    if (isset($_POST['ID']) && !empty($_POST['ID'])) {
        $ID_rev = mysqli_real_escape_string($db, $_POST['ID']);
        $nombre_rev = mysqli_real_escape_string($db, $_POST['nombre']);
        $mail_rev = mysqli_real_escape_string($db, $_POST['mail']);

    //QUERY SQL
        $UPD_CC = "UPDATE Cat_Rev_main
                    SET nombre = '$nombre_rev',
                        mail = '$mail_rev'
                    WHERE ID = $SelID";
        ...
        if ($res_cc) {
            prnMsgV20('Se ha modificado el Revisor # : '.$ID_rev.' ...','success');
        } else {
            $Msg = 'Error al modificar el Revisor #: '.$ID_rev.'';
            $MsgC = 'red';
        }

        $vSaleM = 'OUT';
    } else {
        $Msg = 'ID del Revisor no proporcionado';
        $MsgC = 'red';
    }
}
            ...
            <div class="card text-left">
                <div class="card-body">
                    ...
                    if ($Msg<>'')
                        ...
                        echo '<a href="AP_003_001n.php?A=vNew" class="btn btn-primary" role="button">
                        Agregar
                        <span class="pl-2 fas fa-plus-circle"></span>
                        </a>';
                    echo '<div id="tablaAP_003_001"></div>';
...
if ($_GET['A']=='vNew') {
    $TituloModal = 'Agregar Nuevo Revisor';
    $_POST['estatus'] = 1;
  }

if ($_GET['A']=='vMod') $TituloModal = 'Modificar Revisor';
if ($_GET['A']=='vDet') $TituloModal = 'Agregar Especialidades';
                    ...
                    <div class = "row">
                        <div class="col-sm-12">
                            <div class="slic-form-group row">
                                <label for="nombre" class="col-sm-3 col-slic-label">Nombre Completo</label>
                                <div class = "col-sm-4">
                                    <input  type="text" 
                                            class="slic-input" 
                                            name="nombre"
...
echo'                      <div class="slic-form-group row">
                                <label for="mail" class="col-sm-3 col-slic-label">Mail</label>
                                <div class = "col-sm-4">
                                    <input  type="text"
                                            class="slic-input"
                                            name="mail"
...
/***
* Modal Pantalla de Datos para agregar detalle
*/
// Modal HTML

echo '<div class="modal fade" id="PantallaDatos_especialidad" tabindex="-1" role="dialog">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">' . $TituloModal . '</h5>
...
echo '           <div class="modal-body d-flex justify-content-center align-items-center">
                            <div class="row">
                                <div class="col-sm-12">
                                    <div class="slic-form-group row">
                                        <label for="temas" class="col-sm-12 col-slic-label">Seleccionar Especialidades a Añadir:</label>
                                    </div>
                                    <div class="col-sm-9">';  
                                    ...
                                    foreach ($temasDisponibles as $tema) {
                                        echo '<div class="form-check">
                                                ...
                                                <label class="form-check-label" for="tema_' . $tema['ID_tema'] . '">' . htmlspecialchars($tema['tema']) . '</label>
                                              </div>';
                                    }
...
echo '          <div class="modal-footer">
                ...
                if ($_GET['A'] == 'vDet') 
                    echo '              <input type="Submit" class="btn btn-primary" name="BtnSav_Det" value="Guardar">';
         ...
         <div class="modal-dialog modal-sm">
            <div class="modal-content">
               ...
               <div class="modal-body">
                  <p>Confirme para eliminar el Revisor ID #'.$SelID .'</p>
               </div>
               <div class="modal-footer">
                  <input type=submit class="btn btn-secondary" name=BtnCer data-dismiss="modal" value=Cerrar>
                  <a href="AP_003_001n.php?A=vEli&ID='.$SelID .'" class="btn btn-primary" 
...
// script para activar el modal de seleccion de temas
if ($_GET['A']=='vDet'  and $vSaleM == 'IN') {
    ?>
    <script type="text/javascript">
        $('#PantallaDatos_especialidad').modal('show'); 
    </script>
    <?php
}
...
<script type="text/javascript">
   $(document).ready(function() {
      $('#tablaAP_003_001').load('AP_003_001t.php');
   });
</script>
</body>
</html>
